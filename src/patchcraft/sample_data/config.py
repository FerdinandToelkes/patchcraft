...
def _add_arguments(parser: argparse.ArgumentParser, default_config='../config.yaml'):
    ...
    parser.add_argument('--overlap', type=float, help='Overlap between the patches in percent. This is only used in the validation mode.')
    ...
    # resize
    parser.add_argument('-ts', '--target_size', type=int, help='Target size of the preprocessed patch.')

    # for changing single arguments in training transform config
    # sampling
    parser.add_argument('-rng', '--patch_size_range', type=float, help='Range for randomly scaling the patch size.')
    parser.add_argument('-pr', '--perturbation_range', type=float, help='Range for randomly perturbing the precomputed coordinates while sampling.')
    # rotation: use BooleanOptionalAction to be able to pass --no-rotation and get boolean values
    parser.add_argument('--rotation', action=argparse.BooleanOptionalAction, help='Whether to randomly rotate the patch or not (pass --no-rotation).')
...
class Config():
    ...
    def update_training_transform_config(self, args: argparse.Namespace, config: dict) -> dict:
        ...
        # Define a list of transform argument names and their corresponding config keys
        transform_arguments = [
            ('patch_size_range', ['sampling', 'patch_size_range']),
            ('perturbation_range', ['sampling', 'perturbation_range']),
